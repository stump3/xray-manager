...
**`modules/08-protocols.sh`** — `proto_vless_splithttp_tls`: `cert_check`
выполнялся после `kset` и построения `ib` (строки 1095–1113 оригинала).
При отсутствии сертификата ключи записывались в `.keys`, inbound не добавлялся,
состояние становилось мусорным. `cert_check` перенесён до первого `kset`.

**`modules/14-telemt.sh`** — проверка занятости порта MTProto (строка 299)
была однократной: пользователь мог ввести занятый порт повторно и установка
продолжалась на конфликтующем порту. Заменена на цикл `while`.

### 🟡 Улучшено (UX)

**`modules/08-protocols.sh`** — после успешного добавления VLESS+WS, VLESS+gRPC,
VLESS+HTTPUpgrade теперь показывается подсказка с готовым `location`-блоком для
nginx (`proxy_pass` / `grpc_pass`). Отображается только если обнаружен
`/root/.xray-mgr-install` (установка через `install.sh`).
