...
**`modules/99-main.sh` — менеджер открывался напрямую без проверки установки Xray**

После `git clone` команда `sudo xray-manager` (или `./xray-manager.sh`) сразу
открывала главное меню даже если Xray-core не был установлен. Статус показывал
`xray ✗ — не установлен`, все пункты меню вели к ошибкам или crash.

Исправлено: добавлен `_preflight_check()` — вызывается перед `main_menu()`.
Если `XRAY_BIN` не является исполняемым файлом:
- Ищет `scripts/install.sh` в 4 стандартных местах (рядом со скриптом,
  текущая директория, `/opt/xray-manager`, `/root/xray-manager-v2`)
- При нахождении — предлагает запустить установку через `exec bash`
- При отсутствии — печатает инструкции и выходит

**`modules/99-main.sh` — crash при выборе любого пункта меню без полного монолита**

Функции `ib_list`, `ib_users_count`, `_sub_is_running`, `menu_protocols`,
`menu_manage`, `menu_system`, `_init_limits_file`, `check_limits` вызывались
в `99-main.sh`, но определены в модулях 05–12 которые отсутствовали в архиве.
При выборе пункта меню: `bash: menu_protocols: command not found`.

Исправлено: добавлены stubs через `declare -f` — если реальная функция не
загружена, подставляется заглушка. Меню-stubs показывают сообщение
«переустановите xray-manager» вместо аварийного завершения.

**`modules/03-system.sh` — `xray_ver()` возвращал «—» на установленном Xray**

`"$XRAY_BIN" -version` — флаг с дефисом не распознаётся Xray-core ≥24.x.
Корректная команда: `xray version` (без дефиса). При этом `install_xray_core()`
и `update_xray_core()` в `04-xray-core.sh` уже использовали правильный вариант.
В главном меню версия всегда отображалась как «—» даже с запущенным Xray.

Исправлено: `"$XRAY_BIN" -version` → `"$XRAY_BIN" version`.

**`modules/03-system.sh` — `xray_ok()` не проверяла исполняемость бинарника**

`[[ -f "$XRAY_BIN" ]]` возвращает true для любого файла, включая повреждённый
или без бита исполнения. `_preflight_check()` считал бы xray установленным
при наличии пустого файла по пути `XRAY_BIN`.

Исправлено: `-f` → `-x` (проверка исполняемого файла).
...
**`scripts/install.sh` — шаги 3 и 4 (nginx, certbot) выполнялись при режиме `xray-direct`**

Флаг `NGINX_NEEDED=false` устанавливался при выборе варианта 3, но шаги 3
(«Настройка Nginx») и 4 («TLS-сертификат») не имели охраны по этому флагу.
Nginx пытался настроиться без бинарника, certbot запускался без nginx —
оба шага падали или зависали.

Исправлено: оба шага обёрнуты в `if [[ "$NGINX_NEEDED" == "true" ]]`.
В режиме `xray-direct` выводится `→ шаг пропущен` и управление
переходит сразу к шагу 5 (установка Xray-core).
...
**`modules/02-ui.sh` — символ `─` рендерился как `█` мусор в SSH без UTF-8 локали**

`_sep()` использовала `printf '%*s' "$w" "" | tr ' ' '─'`. Команда `tr`
обрабатывает байты, а не символы — трёхбайтовая последовательность UTF-8
символа `─` (E2 94 80) подставлялась побайтово. На терминалах без UTF-8
или с неправильной локалью каждый байт рендерился отдельно → полный экран мусора.

Убраны полноширинные разделители. Новый стиль: заголовок секции — жирный
цветной текст с короткой (48 символов) подчёркивающей линией через
`printf '%.0s─' {1..48}` — bash brace expansion, не `tr`, проблема исчезает.

**`modules/12-system.sh` — `do_remove_all()` не выполняла удаление**

Двойной confirm: сначала `confirm "Вы уверены?"`, затем `confirm_word "УДАЛИТЬ"`.
Промпт `confirm_word` был невидим за мусорными строками. Пользователь
нажимал Enter → пустая строка не совпадала с `УДАЛИТЬ` → `return` без действий.

Убран первый confirm. Остался один явный `printf` + `read` прямо
из `/dev/tty` с чётким текстом — промпт всегда виден.
