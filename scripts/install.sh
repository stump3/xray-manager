...
# Снять apt lock если держит фоновый процесс (unattended-upgrades, apt-daily)
# Актуально для свежих серверов где apt-daily запускается сразу после ребута
systemctl stop unattended-upgrades apt-daily.service apt-daily-upgrade.service 2>/dev/null || true
systemctl kill --kill-who=all apt-daily.service apt-daily-upgrade.service 2>/dev/null || true
# Ждать освобождения lock-файла (до 60 сек)
_apt_wait=0
...
# Убрать артефакты предыдущих установок до nginx -t
...
if [[ -n "$_port80_owner" && "$_port80_owner" != "nginx" ]]; then
    ...
    warn "Останови ${_port80_owner}: systemctl stop ${_port80_owner}"
    exit 1
fi
...
# Тест конфига с читаемым выводом при ошибке
if ! nginx -t 2>/tmp/nginx_test.log; then
    err "nginx -t провалился:"
    cat /tmp/nginx_test.log >&2
    exit 1
fi
systemctl restart nginx || {
    err "nginx не запустился. Диагностика:"
