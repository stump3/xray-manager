...
> Xray Manager v2.9.1 — архитектура, дизайн-решения, протоколы данных.
...
### 15. Надёжность install-flow: preflight + APT-обёртки + dry-run

**Проблема.** Прямые вызовы `apt-get` при недоступном внешнем репозитории приводят к задержкам и retry-шуму. `dpkg` может зависнуть на интерактивном вопросе о конфигурационном файле (например, при обновлении `nginx.conf`).

**Решение.** Три уровня защиты:

1. **`preflight_repo_check url name`** — HEAD-запрос с таймаутом 12 с перед добавлением любого внешнего `.list`-файла. При недоступности: `warn` + пропуск репозитория, установка продолжается с системными пакетами. Применяется к `nginx.org` перед добавлением его репозитория.

2. **APT-обёртки** — все прямые `apt-get` заменены на три функции:
   - `apt_update_quiet` — `Acquire::Retries=3`, `Timeout=15` (http + https).
   - `apt_install_quiet …` — `DEBIAN_FRONTEND=noninteractive`, `--force-confdef`, `--force-confold`.
   - `apt_remove_quiet …` — аналогично для удаления.

3. **`run_step desc cmd …`** — обёртка над командой. В `DRY_RUN=true` печатает `[dry-run] desc: cmd` и возвращает 0; в боевом режиме просто запускает команду. Все APT-обёртки вызывают `run_step` внутри.

**Dry-run.** Флаг `--dry-run` разбирается при старте скрипта. После ввода и подтверждения параметров — полный план установки (update, preflight, install, certbot, nginx, xray, systemctl) без единого изменения в системе. Позволяет проверить план перед применением на продакшн-сервере.
...
### 16. Безопасные подтверждения деструктивных операций

**Проблема.** Двойной `y/n`-confirm в `do_remove_all()` не защищает от случайного нажатия Enter — у `confirm()` default `n`, но при быстром двойном Enter сначала отменяется, а при `[Y/n]` (где default `y`) немедленно подтверждается.

**Решение.** `confirm_word(phrase)` в `02-ui.sh`: просит ввести конкретное слово (`УДАЛИТЬ`), возвращает 0 только при точном совпадении строки. Случайный Enter, пробел или опечатка — операция отменяется.

```bash
confirm_word "УДАЛИТЬ" || { info "Операция отменена"; return; }
```

Функция переиспользуемая — принимает произвольную фразу, что позволяет масштабировать паттерн на другие деструктивные операции (например, `confirm_word "СБРОСИТЬ"` для reset-конфига).

**`render_status_bar(xray, nginx, sub, ip)`** — дополнительная UI-функция в `02-ui.sh`. Принимает уже сформированные строки статуса каждого компонента и отображает их единой строкой через `box_row`. Используется в `99-main.sh` для отображения состояния Xray, nginx и подписки одновременно.
...
| Код | Место | Причина | Статус |
|---|---|---|---|
| SC2034 | Весь скрипт | Динамический `printf -v` — переменные используются | подавлено через `-e` |
| SC2086 | jq-фильтры | Намеренно — jq требует нераскрытые переменные | подавлено через `-e` |
